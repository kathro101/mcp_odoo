...
class SchemaDiscovery:
    ...
    def _detect_auto_generated(fields: dict[str, FieldInfo], model_name: str) -> None:
        ...
        but has no compute method is likely auto-generated by an Odoo sequence.
        Fields with a 'readonly' attribute set by the model are also candidates.

        Marked fields are excluded from create_fields and required_fields
        later in _classify_fields — Claude should never be asked to fill them.
    ...
    def _classify_fields(fields: dict[str, FieldInfo]) -> tuple[list[str], list[str], list[str]]:
        ...
        for fname, fi in fields.items():
            # Skip auto-generated fields — they should never appear
            # in create or required lists
